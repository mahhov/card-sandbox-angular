...
export class ScriptRepository {
    ...
    public getAll(): Promise<ScriptEntity[]> {
        return this.http.get<ScriptEntity[]>(environment.nodeUrl + 'scripts').toPromise()
    ...
    public getAllByUser(user: string): Promise<ScriptEntity[]> {
        return this.http.get<ScriptEntity[]>(environment.nodeUrl + 'script/' + user).toPromise()
            .catch((error: any): void => {
                console.log(error);
            });
    }
    ...
    public get(user: string, name: string): Promise<ScriptEntity> {
        return this.http.get<ScriptEntity>(environment.nodeUrl + 'script/' + user + '/' + name).toPromise()
            .catch((error: any): void => {
                console.log(error);
            });
    }
    ...
    public update(token: string, user: string, name: string, body: string): void {
        ...
        this.http.put(environment.nodeUrl + 'script/' + user + '/' + name, body, options).toPromise()
            .catch((error: any): void => {
                console.log(error);
            });
    }
    ...
    public remove(token: string, user: string, name: string): void {
        let headers: HttpHeaders = new HttpHeaders().append('authenticationToken', token);
        let options = {responseType: 'text' as 'text', headers: headers};
        this.http.delete(environment.nodeUrl + 'script/' + user + '/' + name, options).toPromise()
            .catch((error: any): void => {
                console.log(error);
            });
    }
